...
import { useFetchContactInfoById, useUpdateContact } from 'services/hooks/administratorDetails';
...
export const EditContact: React.FC<EditContactModalProps> = ({ isOpen, onClose }) => {
    ...
    const { mutate } = useUpdateContact(sportId, id, {
        onSuccess: (response) => {
    ...
    useEffect(() => {
        if (data) {
            ...
            reset({
                ...
                nationalIdExpiration: data?.payload?.nationalIdExpiration || '',
