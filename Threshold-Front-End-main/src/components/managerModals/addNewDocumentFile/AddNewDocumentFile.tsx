...
import { useAddDocument, useFetchDocumentsById } from 'services/hooks/managerDetails';
...
export const AddNewDocumentFile: React.FC<addNewDocumentModalProps> = ({
    ...
    const { refetch } = useFetchDocumentsById(academyId, id);
    const { mutate } = useAddDocument(academyId, id, {
        onSuccess: (response) => {
