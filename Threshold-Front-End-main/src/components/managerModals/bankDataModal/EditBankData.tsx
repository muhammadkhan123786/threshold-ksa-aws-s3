...
import { useUpdateBankData, useGetBankData } from 'services/hooks/managerDetails';
...
interface EditBankDataForm {
    iban: string;
    bank: any;
    accountHolder: string;
}

export const EditBankData: React.FC<EditBankDataModalProps> = ({ isOpen, onClose }) => {
    ...
    const { data, refetch } = useGetBankData(academyId, id);
    console.log('bank dataaaaaaaaaaaaaa', data?.data?.payload);
    ...
    const { mutate } = useUpdateBankData(academyId, id, {
    ...
    const handleSave: SubmitHandler<EditBankDataForm> = (_data) => {
        setIsLoading(true);
        const mappedData: EditBankDataForm = {
            iban: _data.iban,
    ...
    useEffect(() => {
        if (data) {
            ...
            const selectedBank: any = bankOptions.find(
                (option) => option.value === data?.data?.payload?.bank,
            );

            reset({
                iban: data?.data?.payload?.iban || '',
                bank: selectedBank?.value || null,
                accountHolder: data?.data?.payload?.accountHolder || '',
            });
            methods.clearErrors();
        }
    }, [data, reset]);
