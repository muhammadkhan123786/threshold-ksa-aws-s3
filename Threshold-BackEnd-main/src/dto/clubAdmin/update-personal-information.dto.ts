...
export class UpdatePersonalInformationDto {
    ...
    @ApiProperty({
        description: "The role type of the person.",
        enum: ClubAdminRole,
        example: ClubAdminRole.CLUB_ADMIN_ASSISTANT,
        required: false,
    })
    @IsOptional()
    @IsEnum(ClubAdminRole, { message: "roleType must be a valid enum value" })
    roleType?: ClubAdminRole;
    ...
    @ApiProperty({
        description: "National ID number (رقم الهوية الوطنية/الإقامة)",
        example: "1234567890",
        required: false,
    })
    @IsOptional()
    ...
    @ApiProperty({
        description:
            "National ID expiration date (تاريخ انتهاء الهوية الوطنية/الإقامة)",
        example: "2030-01-01",
        required: false,
    })
    @IsOptional()
